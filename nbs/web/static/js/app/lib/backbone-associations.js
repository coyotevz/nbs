...
//
//  Modified by Augusto Roccasalva

// jshint -W055,-W083,-W004
...
(function(root, factory) {
    ...
    var Associations, Model, Collection, Events, pathChecker, delimiters,
        pathSeparator, sourceModel, sourceKey, endPoints = {};

    var ModelProxies = {};
    var CollectionProxies = {};
    ...
    */
    ...
    // Create local reference `Model, Collection, Event`.
    ...
    */
    ...
    Associations = Backbone.Associations = {
        VERSION: "0.6.1"
    };
    ...
    Associations.Many = Backbone.Many = "Many";
    Associations.One = Backbone.One = "One";
    Associations.Self = Backbone.Self = "Self";
    ...
    // Store original functions to be called in our implementation
    ModelProxies.get = Model.prototype.get;
    ModelProxies.set = Model.prototype.set;
    ModelProxies.trigger = Model.prototype.trigger;
    ModelProxies.toJSON = Model.prototype.toJSON;
    ...
    _.extend(Model.prototype, {
      ...
      on: function(name, callback, context) {
        ...
        var result = Events.on.apply(this, arguments);

        // No optimizacion possible if nested-events is wanted by the application
      ...
      off: function(name, callback, context) {
        ...
        if (Associations.EVENTS_NC) return Events.off.apply(this, arguments);
        ...
        var result = Events.off.apply(this, arguments);
      ...
      get: function(attr) {
        var obj = ModelProxies.get.call(this, attr);
      ...
      _set: function(attributes, options) {
        ...
        if (modelMap) {
          for (modelId in modelMap) {
            ...
            if (!this._setAttr.call(obj.model, obj.data, options)) result = false;
      ...
      _setAttr: function(attributes, options) {
        ...
        if (this.relations) {
          ...
          _.each(this.relations, function(relation) {
            ...
            if (relatedModel && !(relatedModel.prototype instanceof Model))
            ...
            if (map && _.isString(map)) map = map2Scope(map, activationContext);
            ...
            if (attributes[relationKey]) {
              ...
              if (relation.type === Backbone.Many) {
                ...
                if (collectionType && _.isFunction(collectionType) &&
                    (collectionType.prototype instanceof Model))
                ...
                if (collectionType && !(collectionType.prototype instanceof Collection))
                  collectionType = _.isFunction(collectionType) ?
                  collectionType.call(this, relation, attributes) : collectionType;

                if (collectionType && _.isString(collectionType))
                ...
                if (currVal) {
                  ...
                  currVal[relationOptions.reset ? 'reset' : 'set'](
                    val instanceof Collection ? val.models : val, relationOptions);
                ...
                if (!(relatedModel.prototype instanceof Model))
                  throw new Error('specify an associated Model for Backbone.One type');

                data = val instanceof Model ? val : new relatedModel(val, relationOptions);
                ...
                if (currVal && data.attributes[idKey] &&
                  ...
                  currVal._set(val instanceof Model ? val.attributes : val, relationOptions);
                  data = currVal;
                } else {
                  newCtx = true;
                }
        ...
        // Return results for `Model.set`.
        return ModelProxies.set.call(this, attributes, options);
      },
      ...
      _bubbleEvent: function(relationKey, relationValue, eventArguments) {
        ...
        if (relationValue instanceof Collection && (isChangeEvent || eventPath)) {
          // O(n) search :(
      ...
      _createCollection: function(type, context) {
        ...
        if (_.isString(relatedModel)) relatedModel = map2Scope(relatedModel, context);
        // Creates new `Backbone.Collection` and defines model class.
        if (relatedModel && (relatedModel.prototype instanceof Model) || _.isFunction(relatedModel)) {
          collection = new Collection();
          collection.model = relatedModel;
        } else {
          throw new Error('type must inherit from Backbone.Model');
        }
        return collection;
      },
      ...
      trigger: function(name) {
        ...
        if (this._deferEvents) {
          ...
          ModelProxies.trigger.apply(this, arguments);
        }
      },
      ...
      toJSON: function(options) {
        ...
        if (!this.visited) {
          this.visited = true;
          // Get json representation from `Model.toJSON`.
          json = ModelProxies.toJSON.apply(this, arguments);
            ...
            _.each(this.relations, function(relation) {
              ...
              if (serialize) {
                ...
                if (serialize_keys.length) {
                  if (options) options.serialize_keys = serialize_keys;
                  else options = { serialize_keys: serialize_keys };
                }
      ...
      // Call this if you want to set an associated `Model` to a falsy value like undefined/null directly.
    ...
    _.each(['set', 'remove', 'reset'], function (method) {
      CollectionProxies[method] = Collection.prototype[method];

      Collection.prototype[method] = function(models, options) {
        // Short-circuit if this collection doesn't hold `Models` with associations
        if (this.model.prototype.relations && this.parents) {
          // Find a map function if available and perform a transformation
          arguments[0] = map2models(this.parents, this, models);
        }
        return CollectionProxies[method].apply(this, arguments);
      };
    });

    // Override trigger to defer events in the object graph.
    CollectionProxies.trigger = Collection.prototype.trigger;
    Collection.prototype.trigger = function (name) {
      if (this._deferEvents) {
        ...
        CollectionProxies.trigger.apply(this, arguments);
      }
    };

    // Attach process pending event functionality on collections as well. Re-use from `AssociatedModel`
    Collection.prototype._processPendingEvents = Model.prototype._processPendingEvents;
    Collection.prototype.on = Model.prototype.on;
    Collection.prototype.off = Model.prototype.off;
